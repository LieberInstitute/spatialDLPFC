...
ds_geneList <- list(
    DE_DS_DS.Up = ds$ensemblID[ds$fold_difference_log2 > 0 & ds$qval < 0.05],
...
twas_geneList <- list(
    TWAS_BS2_SCZ.Up = tt_dlpfc$ensemblID[tt_dlpfc$TWAS.Z > 0 & tt_dlpfc$TWAS.FDR < 0.05],
...
for (i in seq(along = eb0_list)) {
    ...
    tabList <- mclapply(geneList_present, function(g) {
        tt <- table(
            Set = factor(names(layer) %in% g, c(FALSE, TRUE)),
...
enrichLong_ASD <- enrichLong[enrichLong$ID %in%
    c(
        "Gene_SFARI_all", "Gene_Satterstrom_ASC102.2018",
    ...
    ), ] # why doesn't this list include the BirnBaum asd sets?
...
enrichLong_ASD$LayerFac <- factor(
    as.character(enrichLong_ASD$Layer),
...
customLayerEnrichment <- function(
    ...
    wide_p <- t(round(wide_p[
        ,
        c("1.Pval", "2.Pval", "3.Pval", "4.Pval", "5.Pval", "6.Pval", "7.Pval", "8.Pval", "9.Pval")
    ], 2))

    wide_or <- enrichTab[groups, grep("OR", colnames(enrichTab))]
    wide_or <- round(t(wide_or[
        ,
        c("1.OR", "2.OR", "3.OR", "4.OR", "5.OR", "6.OR", "7.OR", "8.OR", "9.OR")
    ]), 1)
    if (enrichOnly) wide_p[wide_or < 1] <- 0
    wide_or[wide_p < ORcut] <- ""

    image.plot(
        x = seq(0, ncol(wide_p), by = 1), y = layerHeights, z = as.matrix(t(wide_p)),
...
xlabs <- ss(groups, "_", 3)
customLayerEnrichment(enrichTab, groups, xlabs,
    enrichOnly = TRUE,
...
gst_tab <- apply(t0_contrasts, 2, function(tt) {
    sapply(geneList_present, function(g) {
        geneSetTest(
            index = rownames(t0_contrasts) %in% g,
...
for (i in 1:ncol(t0_contrasts)) {
    plot(density(t0_contrasts[!g, i]),
        lwd = 2, col = "black", xlab = "",
...
for (i in 1:ncol(t0_contrasts)) {
    ...
    legend(ll, c("BG", "102 All", "53 ASD", "49 DDID"),
        bty = "n",
