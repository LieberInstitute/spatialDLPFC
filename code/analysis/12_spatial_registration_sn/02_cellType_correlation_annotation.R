...
layer_compare |> filter(grepl("Excit", cluster), layer_confidence == "good") |> arrange(layer_label) |> mutate(change = layer_label != layer_label_old)
...
anno_matrix <- layer_anno |> 
  ...
  mutate(layers = gsub("\\*","",gsub("^([1-9])","L\\1",layers)),
...
fix_layer_order <- function(l){
  ...
  if(all(l == "WM")) return(paste0("WM",star))
