...
layer_anno_easy$layer |>
    left_join(layer_anno_strict$layer |> select(cluster, strict_label = layer_label)) |>
    filter(layer_label != strict_label)
...
layer_anno_long <- cellType_layer_annotations |>
    ...
    mutate(
        layer_combo = factor(ifelse(is.na(layer_combo), layer_long, as.character(layer_combo)),
