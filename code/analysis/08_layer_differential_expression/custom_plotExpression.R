...
#' )
custom_plotExpression <- function(sce, genes, assay = "logcounts", cat, highlight = "highlight", fill_colors = NULL, title = NULL) {
    cat_df <- as.data.frame(colData(sce))[, c(cat, highlight), drop = FALSE]
    
    expression_long <- reshape2::melt(as.matrix(assays(sce)[[assay]][genes, ,drop = FALSE]))
    cat <- cat_df[expression_long$Var2, ]
    colnames(cat) <- c("cat", "highlight")
    expression_long <- cbind(expression_long, cat)

    expression_violin <- ggplot(data = expression_long, aes(x = cat, y = value, fill = cat)) +
        geom_violin(aes(color = highlight), scale = "width") +
        geom_jitter(size = 1) +
