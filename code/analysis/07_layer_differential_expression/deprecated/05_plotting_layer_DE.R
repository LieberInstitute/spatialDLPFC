...
load(file = here::here("processed-data", "rdata", "spe", "07_layer_differential_expression", paste0("res_layer_k", k, ".Rdata")))

## create violin plots for differentially expressed genes
# load(file = here::here("processed-data","rdata","spe","07_layer_differential_expression",paste0("res_layer_k",k,".Rdata")))


pdf(file = here::here("plots", "07_layer_differential_expression", paste0("topTags_violin_k", k, ".pdf")), height = 8, width = 8)
print(
    plotExpression(logNormCounts(spe_pseudo, size.factors = NULL),
        features = rownames(topTags(res)),
