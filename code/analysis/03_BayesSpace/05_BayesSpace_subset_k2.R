...
sample_order <- c("Br8667_mid", "Br6522_ant", "Br6432_ant")

## Get the k = 2 plots for these 3 samples
pk <- vis_grid_clus(
    spe = spe,
...
## Repeat, but with alpha = 0 to just get the histology
pk2 <- vis_grid_clus(
    spe = spe,
    clustervar = paste0("bayesSpace_harmony_", k),
    sort_clust = FALSE,
...
pk2 <- lapply(sample_order, function(sampleid) {
    p <- pk2[[sampleid]]
    p + theme(
        legend.position = "none",
        plot.title = ggplot2::element_text(size = 40)
...
pdf(
    file = here::here(
        ...
        "03_BayesSpace",
        paste0(
            "polychrome_vis_grid_clus_sfigu_BayesSpace_k",
            k,
            "_Br8667_mid_Br6522_ant_Br6432_ant.pdf"
        )
    ),
    height = 5 * 2,
    width = 5 * 3
)
print(cowplot::plot_grid(plotlist = c(pk2, pk)))
dev.off()
