...
#' @examples
#'
#' test_anno_a <- data.frame(
#'     cluster = c("Astro", "Excit", "Oligo"),
#'     layer_confidence = "good",
#'     layer_label = paste0("SpD", 1:3),
#'     Dataset = "A"
#' )
#'
#' test_anno_b <- data.frame(
#'     cluster = c("Astro", "Excit", "Excit", "Oligo"),
#'     layer_confidence = "good",
...
registration_dot_plot2 <- function(
    ...
    layer_df <- annotation_df |>
        ...
        dplyr::distinct()
    ct_df <- annotation_df %>%
        select(any_of(c(cluster_by, "ct_cat"))) |>
        dplyr::distinct()
