...
res_list <- readRDS(here::here("processed-data", "rdata", "spe", "13_nnSVG", "pairwise", "res_list_merged_pairs_2.rds"))
...
saveRDS(num_sig_svgs, file = here::here("processed-data", "rdata", "spe", "13_nnSVG", "pairwise", "num_sig_svgs_combine_2.rds"))
...
write.csv(df_summary, file = here::here("processed-data", "rdata", "spe", "13_nnSVG", "pairwise", "avg_rank_combine_2.csv"), row.names = FALSE)
...
for (i in SVG_search) {
    vis_grid_gene(
        spe = spe,
        geneid = i,
        pdf = here::here("plots", "13_nnSVG", "pairwise", "combine_2", paste0(gsub("; ", "_", i), ".pdf")),
        assayname = "logcounts",
        minCount = 0,
        cont_colors = c(
            "aquamarine4",
