...
def network_top6(adata, tgt):
    ...
    plt.savefig("../../../plots/gene-risk-LR-analysis/03-colocalisation_analysis/%s_c2l_6cells_heatmap.pdf" % (tgt), dpi = 150, bbox_inches = 'tight')
    plt.show()
    plt.clf()
    adjmat_notnorm.to_csv("../../../processed-data/gene-risk-LR-analysis/03-colocalisation_analysis/%s_c2l_6cells_adjacencymatrix_notnormalised.csv" % (tgt))
    adjmat.to_csv("../../../processed-data/gene-risk-LR-analysis/03-colocalisation_analysis/%s_c2l_6cells_adjacencymatrix.csv" % (tgt))
    return adjmat


###--------------------------------------------RUN ANALYSIS FOR TOP 6 CELLS
...
LRadjmat = network_top6(LR, 'LR_'+sys.argv[1]+'-'+sys.argv[2])
print('Calculating control neighbourhood...')
ctrladjmat = network_top6(ctrl, 'Ctrl_'+sys.argv[1]+'-'+sys.argv[2])
...
plt.savefig("../../../plots/gene-risk-LR-analysis/03-colocalisation_analysis/ratio_%s_c2l_6cells_network.pdf" % ('Ctrl'+sys.argv[1]+'-'+sys.argv[2]), dpi = 150, bbox_inches = 'tight')
plt.show()
plt.clf()
...
plt.savefig("../../../plots/gene-risk-LR-analysis/03-colocalisation_analysis/ratio_%s_c2l_6cells_network_celllabels.pdf" % ('Ctrl'+sys.argv[1]+'-'+sys.argv[2]), dpi = 150, bbox_inches = 'tight')
plt.show()
plt.clf()
