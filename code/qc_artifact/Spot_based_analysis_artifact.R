...
Br6522_ant <- read10xVisiumWrapper(samples = paste0(datDir, "NextSeq/Round3/DLPFC_Br6522_ant_manual_alignment_all/outs/"),
...
Br6522_mid <- read10xVisiumWrapper(samples = paste0(datDir, "NextSeq/Round3/DLPFC_Br6522_mid_manual_alignment_all/outs/"),
...
Br8667_post <- read10xVisiumWrapper(samples = paste0(datDir, "NextSeq/Round3/DLPFC_Br8667_post_manual_alignment_all/outs/"),
...
compare_gene_prop <- function(normal, all, title){
  plot_df <- data.frame("Mean_expr" = rowMeans(all@assays@data$logcounts), 
  ...
  p1 <- ggplot(plot_df, aes(x = Mean_expr_normal, y = Mean_expr)) + geom_point(colour = "#00BFC4", alpha = 0.5) +
        theme_classic() + xlab("Mean (excl. artifacts)") + ylab("Mean (all spots)") + 
        geom_abline(intercept = 0, slope = 1, lty = 2) + theme(axis.title = element_text(size = 10), plot.title = element_text(face = "bold", size = 10))
  p2 <- ggplot(plot_df, aes(x = Var_expr_normal, y = Var_expr)) + geom_point(colour = "#00BFC4", alpha = 0.5) +
    theme_classic() + xlab("Variance (excl. artifacts)") + ylab("Variance (all spots)") + 
    geom_abline(intercept = 0, slope = 1, lty = 2) + theme(axis.title = element_text(size = 10)) + ggtitle("")
...
p1 <- ggplot(layer_mat, aes(x = Var1, y = value, fill = Var2)) + 
  ...
  scale_fill_manual(name = "Layers", values = c("Layer 1" = "#F0027F", "Layer 2" = "#377EB8", "Layer 3" = "#4DAF4A", 
                                                "Layer 4" = "#984EA3", "Layer 5" = "#FFD700", "Layer 6" = "#FF7F00", 
                                                "WM" = "#1A1A1A", "NA" = "transparent")) + xlab("") + ylab("Number of spots") +
  ggtitle(title) + theme(plot.title = element_text(face = "bold"))

p2 <- ggplot(artifact_mat, aes(x = Var1, y = value, fill = Var2)) + 
  geom_bar(position="fill", stat="identity") + theme_classic() + 
  scale_fill_manual(name = "", values = c("None" = "#619CFF", "Artifact" = "#F8766D")) + xlab("") + ylab("Fraction of spots") +
...
viz_umap <- function(sobj, title){
  ...
  ggplot(df, aes(x = UMAP1, y = UMAP2, colour = Layers)) + geom_point(size = 0.8, alpha = 0.7) + theme_bw() +
    scale_color_manual(name = "Layers", values = layer_palette) + facet_wrap(~Artifact) + ggtitle(title) + 
...
ggplot(aes(x=variable, y=PC, fill=value), data=Br6522_ant_anova) + geom_tile() + scale_fill_gradient(low="white", high="#416A1D") +
  labs(y=NULL, x=NULL, fill="R-squared") + geom_text(aes(label = round(value, 2)), size = 3) +
  ...
  ggtitle("Br6522 Anterior") + theme(plot.title = element_text(face="bold"))
dev.off()

pdf("/data/abattle4/prashanthi/dewrinkler/figures/fig_S5/Br6522_mid_PC_anova.pdf", width = 5, height = 3.5)
ggplot(aes(x=variable, y=PC, fill=value), data=Br6522_mid_anova) + geom_tile() + scale_fill_gradient(low="white", high="#416A1D") +
  labs(y=NULL, x=NULL, fill="R-squared") + geom_text(aes(label = round(value, 2)), size = 3) +
  theme_classic() + theme(axis.line = element_blank(), axis.ticks.length = unit(0, "lines")) +
  scale_y_discrete(limits = c("PC1", "PC2", "PC3", "PC4", "PC5", "PC6", "PC7", "PC8", "PC9", "PC10")) +
  ggtitle("Br6522 Middle") + theme(plot.title = element_text(face="bold"))
dev.off()

pdf("/data/abattle4/prashanthi/dewrinkler/figures/fig_S5/Br8667_post_PC_anova.pdf", width = 5, height = 3.5)
ggplot(aes(x=variable, y=PC, fill=value), data=Br8667_post_anova) + geom_tile() + scale_fill_gradient(low="white", high="#416A1D") +
  labs(y=NULL, x=NULL, fill="R-squared") + geom_text(aes(label = round(value, 2)), size = 3) +
  theme_classic() + theme(axis.line = element_blank(), axis.ticks.length = unit(0, "lines")) +
  scale_y_discrete(limits = c("PC1", "PC2", "PC3", "PC4", "PC5", "PC6", "PC7", "PC8", "PC9", "PC10")) +
  ggtitle("Br8667 Posterior") + theme(plot.title = element_text(face="bold"))
dev.off()
...
plot_DE_genes <- function(df_list, opt = "all", quantile_thresh = 0.95){
  ...
  ggplot(plot_df, aes(x= Layer, y=genes_ordered, size=Absolute_logFC, color=FDR)) + geom_point(alpha = 0.8) + 
    theme_bw() + xlab("") + ylab("") + scale_colour_gradient(low = "#3D07F4", high = "#E2DBF8") +
    scale_size_continuous(name = "|log FC|") + theme(panel.grid.major = element_line(colour = "black",linewidth = 0.2, linetype="dashed")) + theme(
...
plot_DE_violin_plot <- function(df_list, opt = "all"){
  ...
  plot_df$pvalue_cat <- factor(plot_df$pvalue_cat, levels = c("FDR > 0.1", "0.05 < FDR < 0.1", "0.01 < FDR < 0.05", "0.001 < FDR < 0.01", 
                                                              "FDR < 0.001"))
 ...
 ggplot(plot_df.summary, aes(x=Layer, y = median)) + xlab("") + ylab("Log Fold-change") +
   geom_hline(yintercept = -0.1, lty = 2, linewidth = 0.3) + geom_hline(yintercept = 0.1, lty = 2, linewidth = 0.3) +
   theme_classic() + geom_point(aes(color = pvalue_cat), position = position_dodge(0.5)) +
   geom_errorbar(aes(ymin = lowerLim, ymax = upperLim, color = pvalue_cat), position = position_dodge(0.5), width = 0) + 
   ylim(c(-0.5, 0.5)) + scale_color_manual(name = "", values = c("FDR > 0.1" = "#C3B2F9",
                                                                 "0.05 < FDR < 0.1" = "#BBA8F8",
...
Br6522_ant_out_all <- scran::combineMarkers(
  de.lists = Br6522_ant_out$statistics[paste(Br6522_ant_out$pairs$first, Br6522_ant_out$pairs$second) %in% c("Layer 1_0 Layer 1_1", 
                                                                                                             "Layer 2_0 Layer 2_1", "Layer 3_0 Layer 3_1",
                                                                                                             "Layer 4_0 Layer 4_1", "Layer 5_0 Layer 5_1", 
                                                                                                             "Layer 6_0 Layer 6_1", "WM_0 WM_1")],
  pairs = Br6522_ant_out$pairs[paste(Br6522_ant_out$pairs$first, Br6522_ant_out$pairs$second) %in% c("Layer 1_0 Layer 1_1", 
                                                                                                     "Layer 2_0 Layer 2_1", "Layer 3_0 Layer 3_1", 
                                                                                                     "Layer 4_0 Layer 4_1", "Layer 5_0 Layer 5_1",         
                                                                                                     "Layer 6_0 Layer 6_1", "WM_0 WM_1"), ],
  pval.type = "all"
...
Br6522_mid_out_all <- scran::combineMarkers(
  de.lists = Br6522_mid_out$statistics[paste(Br6522_mid_out$pairs$first, Br6522_mid_out$pairs$second) %in% c("Layer 1_0 Layer 1_1", 
                                                                                                             "Layer 2_0 Layer 2_1", "Layer 3_0 Layer 3_1",
                                                                                                             "Layer 4_0 Layer 4_1", "Layer 5_0 Layer 5_1", 
                                                                                                             "Layer 6_0 Layer 6_1", "WM_0 WM_1")],
  pairs = Br6522_mid_out$pairs[paste(Br6522_mid_out$pairs$first, Br6522_mid_out$pairs$second) %in% c("Layer 1_0 Layer 1_1", 
                                                                                                     "Layer 2_0 Layer 2_1", "Layer 3_0 Layer 3_1", 
                                                                                                     "Layer 4_0 Layer 4_1", "Layer 5_0 Layer 5_1",         
                                                                                                     "Layer 6_0 Layer 6_1", "WM_0 WM_1"), ],
  pval.type = "all"
...
Br8667_post_out_some <- scran::combineMarkers(
  de.lists = Br8667_post_out$statistics[paste(Br8667_post_out$pairs$first, Br8667_post_out$pairs$second) %in% c("Layer 1_0 Layer 1_1", 
                                                                                                                "Layer 2_0 Layer 2_1", "Layer 3_0 Layer 3_1",
                                                                                                                "Layer 4_0 Layer 4_1", "Layer 5_0 Layer 5_1", 
                                                                                                                "Layer 6_0 Layer 6_1", "WM_0 WM_1")],
  pairs = Br8667_post_out$pairs[paste(Br8667_post_out$pairs$first, Br8667_post_out$pairs$second) %in% c("Layer 1_0 Layer 1_1", 
                                                                                                        "Layer 2_0 Layer 2_1", "Layer 3_0 Layer 3_1", 
                                                                                                        "Layer 4_0 Layer 4_1", "Layer 5_0 Layer 5_1",         
                                                                                                        "Layer 6_0 Layer 6_1", "WM_0 WM_1"), ],
  pval.type = "all"
...
Br6522_ant_out_all[[1]]$Layer <- "Layer 1"
Br6522_ant_out_all[[2]]$Layer <- "Layer 2"
Br6522_ant_out_all[[3]]$Layer <- "Layer 3"
Br6522_ant_out_all[[4]]$Layer <- "Layer 4"
Br6522_ant_out_all[[5]]$Layer <- "Layer 5"
Br6522_ant_out_all[[6]]$Layer <- "Layer 6"
Br6522_ant_out_all[[7]]$Layer <- "WM"
Br6522_ant_out_all <- lapply(Br6522_ant_out_all, function(df){
  df$gene <- rownames(df)
  df <- df[df$FDR < 0.01, ]
  df[ ,4] <- NULL
  colnames(df) <- c("p_value", "FDR", "logFC", "Layer", "Gene")
  df
})
Br6522_ant_out_df <- do.call(rbind, Br6522_ant_out_all)
Br6522_ant_out_df <- data.frame(Br6522_ant_out_df)
rownames(Br6522_ant_out_df) <- c(1:dim(Br6522_ant_out_df)[1])
Br6522_ant_out_df <- Br6522_ant_out_df[ ,c("Layer", "Gene", "logFC", "p_value", "FDR")]
Br6522_ant_out_df <- cbind("Sample" = "Br6522_ant", Br6522_ant_out_df)

Br6522_mid_out_all[[1]]$Layer <- "Layer 1"
Br6522_mid_out_all[[2]]$Layer <- "Layer 2"
Br6522_mid_out_all[[3]]$Layer <- "Layer 3"
Br6522_mid_out_all[[4]]$Layer <- "Layer 4"
Br6522_mid_out_all[[5]]$Layer <- "Layer 5"
Br6522_mid_out_all[[6]]$Layer <- "Layer 6"
Br6522_mid_out_all[[7]]$Layer <- "WM"
Br6522_mid_out_all <- lapply(Br6522_mid_out_all, function(df){
  df$gene <- rownames(df)
  df <- df[df$FDR < 0.01, ]
  df[ ,4] <- NULL
  colnames(df) <- c("p_value", "FDR", "logFC", "Layer", "Gene")
...
Br6522_mid_out_df <- do.call(rbind, Br6522_mid_out_all)
Br6522_mid_out_df <- data.frame(Br6522_mid_out_df)
rownames(Br6522_mid_out_df) <- c(1:dim(Br6522_mid_out_df)[1])
Br6522_mid_out_df <- Br6522_mid_out_df[ ,c("Layer", "Gene", "logFC", "p_value", "FDR")]
Br6522_mid_out_df <- cbind("Sample" = "Br6522_mid", Br6522_mid_out_df)

Br8667_post_out_all[[2]]$Layer <- "Layer 2"
Br8667_post_out_all[[3]]$Layer <- "Layer 3"
Br8667_post_out_all[[4]]$Layer <- "Layer 4"
Br8667_post_out_all[[5]]$Layer <- "Layer 5"
Br8667_post_out_all[[6]]$Layer <- "Layer 6"
Br8667_post_out_all <- lapply(Br8667_post_out_all, function(df){
  if(is.null(df)){
    NULL
  }else{
  df$gene <- rownames(df)
...
Br8667_post_out_df <- do.call(rbind, Br8667_post_out_all)
Br8667_post_out_df <- data.frame(Br8667_post_out_df)
rownames(Br8667_post_out_df) <- c(1:dim(Br8667_post_out_df)[1])
Br8667_post_out_df <- Br8667_post_out_df[ ,c("Layer", "Gene", "logFC", "p_value", "FDR")]
Br8667_post_out_df <- cbind("Sample" = "Br8667_post", Br8667_post_out_df)

all_res <- rbind(Br6522_ant_out_df, Br6522_mid_out_df, Br8667_post_out_df)

write.csv(all_res, "/data/abattle4/prashanthi/dewrinkler/tables/summary_stats_differential_expressed_genes_artifact.csv", row.names = FALSE)
