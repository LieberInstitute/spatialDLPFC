...
Br6522_ant_1$Wrinkles <- factor(Br6522_ant_1$Wrinkles, c(
    "Fold 1", "Shear 1", "Shear 2", "Shear 3", "Wrinkle 1", "Wrinkle 2",
    "Wrinkle 3", "Wrinkle 4", "Wrinkle 5", "Wrinkle 6", "Wrinkle 7", "Wrinkle 8",
    "Wrinkle 9"
))
...
Br6522_mid_1$Wrinkles <- factor(Br6522_mid_1$Wrinkles, c(
    "Fold 1", "Shear 1", "Shear 2", "Wrinkle 1", "Wrinkle 2",
    "Wrinkle 3", "Wrinkle 4", "Wrinkle 5", "Wrinkle 6", "Wrinkle 7", "Wrinkle 8",
    "Wrinkle 9", "Wrinkle 10", "Wrinkle 11", "Wrinkle 12", "Wrinkle 13"
))
...
Br8667_post_1$Wrinkles <- factor(Br8667_post_1$Wrinkles, c(
    "Shear 1", "Wrinkle 1", "Wrinkle 2",
    "Wrinkle 3", "Wrinkle 4", "Wrinkle 5", "Wrinkle 6", "Wrinkle 7", "Wrinkle 8",
    "Wrinkle 9", "Wrinkle 11", "Wrinkle 12", "Wrinkle 13"
))
...
make_metadata_layers_plots <- function(sobj, title) {
    ...
    count_df_artifact <- data.frame(
        "Layers" = Layers, "nSpots" = nSpots_artifacts,
        "Artifact" = TRUE
    )
    count_df_normal <- data.frame(
        "Layers" = Layers, "nSpots" = nSpots_normal,
        "Artifact" = FALSE
    )
    count_df <- rbind(count_df_artifact, count_df_normal)
    p0 <- ggplot(count_df, aes(x = Layers, y = nSpots, fill = Artifact)) +
        geom_bar(position = "dodge", stat = "identity") +
        theme_classic() +
        ylab("Number of spots") +
        guides(fill = guide_legend(title = "Artifact")) +
        ggtitle(title)
    ...
    p1 <- ggplot(sobj@meta.data) +
        geom_boxplot(aes(x = Layers, y = nCount_Spatial, fill = is_wrinkle), outlier.size = 0.4) +
        theme_classic() +
        ylab("Library Size") +
        guides(fill = guide_legend(title = "Artifact")) +
        ggtitle(title) +
    ...
    stat.test2 <- stat.test2 %>%
        add_xy_position(x = "Layers", dodge = 0.8)
    p2 <- ggplot(sobj@meta.data) +
        geom_boxplot(aes(x = Layers, y = percent.mt, fill = is_wrinkle), outlier.size = 0.4) +
        theme_classic() +
        ylab("Percentage mitochondrial reads") +
        guides(fill = guide_legend(title = "Artifact")) +
        ggtitle(title) +
        stat_pvalue_manual(
            stat.test2,
    ...
    stat.test3 <- stat.test3 %>%
        add_xy_position(x = "Layers", dodge = 0.8)
    p3 <- ggplot(sobj@meta.data) +
        geom_boxplot(aes(x = Layers, y = nFeature_Spatial, fill = is_wrinkle), outlier.size = 0.4) +
        theme_classic() +
        ylab("Detected genes") +
        guides(fill = guide_legend(title = "Artifact")) +
        ggtitle(title) +
    ...
    stat.test4 <- stat.test4 %>%
        add_xy_position(x = "Layers", dodge = 0.8)
    p4 <- ggplot(sobj@meta.data) +
        geom_boxplot(aes(x = Layers, y = nCells, fill = is_wrinkle), outlier.size = 0.4) +
        theme_classic() +
        ylab("Nuclei detected (DAPI)") +
        guides(fill = guide_legend(title = "Artifact")) +
        ggtitle(title) +
        stat_pvalue_manual(
            stat.test4,
    ...
    stat.test5 <- stat.test5 %>%
        add_xy_position(x = "Layers", dodge = 0.8)
    p5 <- ggplot(sobj@meta.data) +
        geom_boxplot(aes(x = Layers, y = percent_reads_LG, fill = is_wrinkle), outlier.size = 0.4) +
        theme_classic() +
        ylab("Percent reads (Largest gene)") +
        guides(fill = guide_legend(title = "Artifact")) +
        ggtitle(title) +
        stat_pvalue_manual(
            stat.test5,
...
make_metadata_wrinkles_plots <- function(sobj, title, layer) {
    ...
    count_df_L1 <- data.frame(
        "Wrinkles" = Wrinkles, "nSpots" = nSpots_L1,
        "Layer" = "Layer 1"
    )
    count_df_L2 <- data.frame(
        "Wrinkles" = Wrinkles, "nSpots" = nSpots_L2,
        "Layer" = "Layer 2"
    )
    count_df_L3 <- data.frame(
        "Wrinkles" = Wrinkles, "nSpots" = nSpots_L3,
        "Layer" = "Layer 3"
    )
    count_df_L4 <- data.frame(
        "Wrinkles" = Wrinkles, "nSpots" = nSpots_L4,
        "Layer" = "Layer 4"
    )
    count_df_L5 <- data.frame(
        "Wrinkles" = Wrinkles, "nSpots" = nSpots_L5,
        "Layer" = "Layer 5"
    )
    count_df_L6 <- data.frame(
        "Wrinkles" = Wrinkles, "nSpots" = nSpots_L1,
        "Layer" = "Layer 1"
    )
    count_df_WM <- data.frame(
        "Wrinkles" = Wrinkles, "nSpots" = nSpots_L6,
        "Layer" = "Layer 6"
    )

    count_df <- rbind(
        count_df_L1, count_df_L2, count_df_L3,
        count_df_L4, count_df_L5, count_df_L6, count_df_WM
    )
    count_df <- count_df[!count_df$Wrinkles == "None", ]
    p0 <- ggplot(count_df, aes(Wrinkles, nSpots, fill = Layer)) +
        geom_bar(position = "dodge", stat = "identity") +
        theme_classic() +
    ...
    stat.test1 <- stat.test1 %>%
        add_xy_position(x = "Layers", dodge = 0.8)
    p1 <- ggplot(sobj@meta.data) +
        geom_boxplot(aes(x = Layers, y = nCount_Spatial, fill = Wrinkles), outlier.size = 0.4) +
        theme_classic() +
        ylab("Library Size") +
        guides(fill = guide_legend(title = "Artifact")) +
        ggtitle(title) +
    ...
    stat.test2 <- stat.test2 %>%
        add_xy_position(x = "Layers", dodge = 0.8)
    p2 <- ggplot(sobj@meta.data) +
        geom_boxplot(aes(x = Layers, y = percent.mt, fill = Wrinkles), outlier.size = 0.4) +
        theme_classic() +
        ylab("Percentage mitochondrial reads") +
        guides(fill = guide_legend(title = "Artifact")) +
        ggtitle(title) +
    ...
    stat.test3 <- stat.test3 %>%
        add_xy_position(x = "Layers", dodge = 0.8)
    p3 <- ggplot(sobj@meta.data) +
        geom_boxplot(aes(x = Layers, y = nFeature_Spatial, fill = Wrinkles), outlier.size = 0.4) +
        theme_classic() +
        ylab("Detected genes") +
        guides(fill = guide_legend(title = "Artifact")) +
        ggtitle(title) +
    ...
    stat.test4 <- stat.test4 %>%
        add_xy_position(x = "Layers", dodge = 0.8)
    p4 <- ggplot(sobj@meta.data) +
        geom_boxplot(aes(x = Layers, y = nCells, fill = Wrinkles), outlier.size = 0.4) +
        theme_classic() +
        ylab("Nuclei detected (DAPI)") +
        guides(fill = guide_legend(title = "Artifact")) +
        ggtitle(title) +
    ...
    stat.test5 <- stat.test5 %>%
        add_xy_position(x = "Layers", dodge = 0.8)
    p5 <- ggplot(sobj@meta.data) +
        geom_boxplot(aes(x = Layers, y = percent_reads_LG, fill = Wrinkles), outlier.size = 0.4) +
        theme_classic() +
        ylab("Percent reads (Largest gene)") +
        guides(fill = guide_legend(title = "Artifact")) +
        ggtitle(title) +
...
combined_metadata$sample <- factor(combined_metadata$sample, levels = c(
    "Br8667 posterior", "Br8667 posterior (artifact)",
...
p1 <- ggplot(combined_metadata) +
    geom_boxplot(aes(x = Layers, y = nCount_Spatial, fill = sample), outlier.shape = NA) +
    theme_classic() +
    guides(fill = guide_legend(title = "Spot category")) +
    theme(legend.direction = "horizontal", axis.title.y = element_text(size = 10)) +
    ylab("Library Size") +
    xlab("") +
...
p2 <- ggplot(combined_metadata) +
    geom_boxplot(aes(x = Layers, y = nFeature_Spatial, fill = sample), outlier.shape = NA) +
    theme_classic() +
    guides(fill = guide_legend(title = "Spot category")) +
    theme(legend.direction = "horizontal", axis.title.y = element_text(size = 10)) +
...
p3 <- ggplot(combined_metadata) +
    geom_boxplot(aes(x = Layers, y = percent.mt, fill = sample), outlier.shape = NA) +
    theme_classic() +
    guides(fill = guide_legend(title = "Spot category")) +
    theme(legend.direction = "horizontal", axis.title.y = element_text(size = 10)) +
    ylab("Percent mito") +
...
p4 <- ggplot(combined_metadata) +
    geom_boxplot(aes(x = Layers, y = nCells, fill = sample), outlier.shape = NA) +
    theme_classic() +
    guides(fill = guide_legend(title = "Spot category")) +
    theme(legend.direction = "horizontal", axis.title.y = element_text(size = 10)) +
    ylab("#Cells (Vistoseg)") +
...
Br6522_ant_normal_vst_out <- sctransform::vst(Br6522_ant_normal_data,
    latent_var = c("log_umi"), return_gene_attr = TRUE,
    return_cell_attr = TRUE, n_genes = NULL, verbosity = 1
)
Br6522_mid_normal_vst_out <- sctransform::vst(Br6522_mid_normal_data,
    latent_var = c("log_umi"), return_gene_attr = TRUE,
    return_cell_attr = TRUE, n_genes = NULL, verbosity = 1
)
Br8667_post_normal_vst_out <- sctransform::vst(Br8667_post_normal_data,
    latent_var = c("log_umi"), return_gene_attr = TRUE,
    return_cell_attr = TRUE, n_genes = NULL, verbosity = 1
)
...
Br6522_ant_all_vst_out <- sctransform::vst(Br6522_ant_all_data,
    cell_attr = Br6522_ant_additional_params, latent_var = c("log_umi", "is_wrinkle"), return_gene_attr = TRUE,
    return_cell_attr = TRUE, n_genes = NULL, verbosity = 1
)
Br6522_ant_all_vst_out_libSize_only <- sctransform::vst(Br6522_ant_all_data,
    cell_attr = Br6522_ant_additional_params, latent_var = c("log_umi"), return_gene_attr = TRUE,
    return_cell_attr = TRUE, n_genes = NULL, verbosity = 1
)
Br6522_ant_all_vst_out_wrinkle_only <- sctransform::vst(Br6522_ant_all_data,
    cell_attr = Br6522_ant_additional_params, latent_var = c("is_wrinkle"), return_gene_attr = TRUE,
    return_cell_attr = TRUE, n_genes = NULL, verbosity = 1
)
...
Br6522_mid_all_vst_out <- sctransform::vst(Br6522_mid_all_data,
    cell_attr = Br6522_mid_additional_params, latent_var = c("log_umi", "is_wrinkle"), return_gene_attr = TRUE,
    return_cell_attr = TRUE, n_genes = NULL, verbosity = 1
)
Br6522_mid_all_vst_out_libSize_only <- sctransform::vst(Br6522_mid_all_data,
    cell_attr = Br6522_mid_additional_params, latent_var = c("log_umi"), return_gene_attr = TRUE,
    return_cell_attr = TRUE, n_genes = NULL, verbosity = 1
)
Br6522_mid_all_vst_out_wrinkle_only <- sctransform::vst(Br6522_mid_all_data,
    cell_attr = Br6522_mid_additional_params, latent_var = c("is_wrinkle"), return_gene_attr = TRUE,
    return_cell_attr = TRUE, n_genes = NULL, verbosity = 1
)

Br8667_post_additional_params <- data.frame(Br8667_post$is_wrinkle)
colnames(Br8667_post_additional_params) <- "is_wrinkle"
Br8667_post_all_vst_out <- sctransform::vst(Br8667_post_all_data,
    cell_attr = Br8667_post_additional_params, latent_var = c("log_umi", "is_wrinkle"), return_gene_attr = TRUE,
    return_cell_attr = TRUE, n_genes = NULL, verbosity = 1
)
Br8667_post_all_vst_out_libSize_only <- sctransform::vst(Br8667_post_all_data,
    cell_attr = Br8667_post_additional_params, latent_var = c("log_umi"), return_gene_attr = TRUE,
    return_cell_attr = TRUE, n_genes = NULL, verbosity = 1
)
Br8667_post_all_vst_out_wrinkle_only <- sctransform::vst(Br8667_post_all_data,
    cell_attr = Br8667_post_additional_params, latent_var = c("is_wrinkle"), return_gene_attr = TRUE,
    return_cell_attr = TRUE, n_genes = NULL, verbosity = 1
)
...
make_compare_plots <- function(vst_out, vst_out_wrinkle_only, sample_label) {
    wrinkle_only_params_df <- data.frame(rbind(
        cbind(vst_out_wrinkle_only$gene_attr$gmean, vst_out_wrinkle_only$model_pars, "Single gene estimate"),
    ...
    p0 <- ggplot(wrinkle_only_params_df, aes(x = gmean, y = is_wrinkleTRUE, colour = estimate_type)) +
        ...
        guides(colour = guide_legend(title = "Estimate type")) +
    ...
    plot_grid(sample_name,
        ...
        estimate_type_legend,
        rel_widths = c(1, 6, 1), nrow = 1
    )
}
...
plot_params <- function(normal_vst_out, all_vst_out, title) {
    ...
    plot_df <- data.frame(
        "Normal_intercept" = normal_params[, 2],
    ...
    ggplot(plot_df) +
        geom_point(aes(x = Normal_intercept, y = All_intercept, colour = "Single gene estimate"), alpha = 0.5) +
        geom_point(aes(x = Normal_intercept_regularized, y = All_intercept_regularized, colour = "Regularized"), alpha = 0.5) +
        geom_abline(slope = 1, intercept = 0, lty = 2) +
        theme_classic() +
        xlab("Intercept (excl. artifacts)") +
        ylab("Intercept") +
        ggtitle(title) +
        scale_colour_manual(name = "Estimate type", values = c("Regularized" = "#F8766D", "Single gene estimate" = "#00BFC4")) +
        theme(
            plot.title = element_text(face = "bold", size = 14), axis.title = element_text(size = 12), legend.title = element_text(size = 12),
